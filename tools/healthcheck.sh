...
PERSISTENT="dev.nucleus.discord dev.nucleus.whatsapp dev.nucleus.nucleus-dashboard dev.nucleus.bonsai dev.nucleus.noobai com.example.cloudflared com.example.container-service"
...
if pgrep -qf "cloudflared tunnel run"; then PASS "cloudflared tunnel process up"; else FAIL "no cloudflared tunnel running (public URLs down)"; fi
