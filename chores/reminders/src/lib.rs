...
//!
//! This crate is both the `nucleus reminders` subcommand ([`run`]) and the
//! data layer the nucleus-dashboard reminders surface imports. `store` is
//! public so the dashboard shares the exact schema, helpers and invariants
//! the CLI uses, with no SQL-shaped drift.
//!
//! Subcommands:
...
/// Entry point for the `nucleus reminders` subcommand. `args` is the full
...
async fn deliver_skill_fire(
    ...
    // mid-task, the daily-report skill-fire failures, 2026-05-26), 300s ceiling
    ...
    // report (a daily-report skill-fire did, 2026-05-26). Only forward when
