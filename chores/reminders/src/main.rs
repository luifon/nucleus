...
//!
//! Subcommands:
//!   end-of-day            daily end-of-day nudge to wrap up the day (preset)
...
#[derive(Subcommand)]
enum Cmd {
    /// Daily end-of-day nudge to wrap up the day.
    End-of-day,
...
async fn main() -> Result<()> {
    ...
    let cli = Cli::parse();
    match cli.command {
        Cmd::EndOfDay => end_of_day(&settings, &workspace_root).await,
...
async fn end_of_day(settings: &Settings, workspace_root: &Path) -> Result<()> {
    ...
    let mention = settings
        ...
        .map(|id| format!("<@{}> ", id))
        .unwrap_or_default();
    let content = format!("{}⏰ End of day — time to wrap up.", mention);
    ...
    .await?;
    tracing::info!("end-of-day reminder sent (msg {})", msg_id);
