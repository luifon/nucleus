...
- **appends** from the stored offset with the stored carry when the identity is the same, the file is not shorter than the offset, and the fingerprint, re-verified whenever the metadata differs, still matches;
