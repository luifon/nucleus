...
`source_files` records, per file, its identity (device and inode), size, mtime and ctime in nanoseconds, the byte offset read so far, the parser's carry state, and a fingerprint of the part already read: a hash of the first 64 KiB and of the 64 KiB that end at the offset. Per file a refresh:

- **skips** it when identity, size, mtime and ctime are all unchanged to the nanosecond (ctime cannot be set by a program, so a rewrite that restores size and mtime still moves it);
- **appends** from the stored offset with the stored carry when the identity is the same, the file grew past both the stored size and the stored offset, and the fingerprint still matches;
- otherwise **re-reads** it from byte 0 with an empty carry, deleting every observation of that path first: a new file, `--full`, a different inode at the path, a grown file whose fingerprint changed, and any file that did not grow but whose mtime or ctime moved (a rewrite in place at any position, also one between the fingerprint windows).

A read covers the bytes up to the size `fstat` reported before it; a half-written last line and anything appended during the read are left for the next refresh. After the read the descriptor is `fstat`ed again. When its identity, mtime or ctime changed and it did not grow (an in-place write during the read), the file's batch is discarded and the file is reported as not read; the stored state keeps the old metadata, so the next refresh plans it again. Each file's deletions, records and new state commit in one transaction, so a crash leaves all or none of it.

The one change incremental refreshes do not detect: an in-place edit that falls between the two fingerprint windows of a file that also grew since the previous refresh (or grew during the read). The append plan trusts the fingerprint for grown files so that an active transcript is not re-read from byte 0 on every refresh. Claude Code and Codex only append to their transcripts, so this requires another program writing into them; `--full` recovers it.

The store has two layers. Per-file **observations** (`usage_obs`, `cost_runs`, `limit_events`, `rate_snapshots`, keyed by path) record what each file says. **Counted rows** (`usage_rows`) are derived from the observations of every file that contains a response: the observation with the largest output (the final line of a streamed response), then the file whose first line is oldest (the original session rather than a fork that copied it), then the path. Only the keys a refresh touched are re-derived; a key no file observes any more is removed. Every key is derived from content, never from a byte position, and the derivation depends only on the stored observations, so, for every sequence of file changes other than the undetected case above, `nucleus usage refresh --full` produces exactly the rows of the incremental history. A test drives one fixture through appends, a same-size rewrite, truncate-and-regrow, a fork, a resume, Codex counter resets and a replace-by-rename, and compares every counted value against a fresh `--full` read; another edits one response between the fingerprint windows of a file longer than 128 KiB without changing its size.

Memory is bounded: the reader holds one line of at most 32 MiB (a longer line is skipped and counted), and records reach the database in batches of 2,000 through a bounded channel inside the file's transaction. Files that cannot be read, relevant lines that are not valid JSON, and oversized lines are counted per refresh in `refresh_runs` (with the first warnings) and per file in `source_files`; the page shows a partial refresh as `[PARTIAL: …]`, and lines skipped in stored files stay reported until the file changes. Deleted-between-discovery-and-read files are not failures. Discovery never follows a symbolic link below the configured roots, so a link cycle or a link out of the root is never walked, and it records each regular file's device and inode. A file is then opened relative to a descriptor of its root, one component at a time with `O_NOFOLLOW`; the descriptor must be a regular file with the discovered identity, and every read uses it. A path swapped for a link, a directory link, or another file between discovery and reading is refused and reported as a failed file.
...
2. **New counter part.** A run subtracts `D`, what its counter provably already holds from earlier runs. The proof is identity, not a comparison of totals. The counter **continued** an earlier run when that run has the same start time (the same counter), at least one response key appears in both windows, and no category of the earlier counter exceeds this run's; the latest such run is the one continued. Then `D` = that run's totals, background calls included (they are the same records of the same counter), plus any carried responses outside that run's window up to what the counter holds beyond that run's totals and this run's own responses, and `D`'s dollars = that run's `costUSD` plus the extra responses' price share of the rest. Otherwise the counter **restarted**: `D` = the carried responses' tokens, capped per category at `C − O_own`, with dollars in proportion to their table price (their token share for a model without a price); an earlier run's background tokens are never subtracted, because nothing shows they are in this counter. The run's own responses are reserved first because they happened inside this counter, while a carried response can predate a counter reset: a run whose reset counter holds only its own new response keeps that response's tokens and dollars. `C' = C − D`, `costUSD' = costUSD − D$`.
