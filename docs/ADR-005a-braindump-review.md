...
ADR-009 (canvas) explicitly **disclaimed** brain-dump review as a
...
- **Op rewriting in the review loop.** Interpreter selects, doesn't
  ...
  feature small.
- **Canvas review on the dashboard.** ADR-009 already disclaimed this.
...
- ADR-006 — reminders / `outbound_queue` (the table reused by the
  ack helper)
- ADR-009 — canvas; explicitly out-of-scope for brain-dump review
