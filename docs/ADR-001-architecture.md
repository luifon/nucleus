...
| `news/fetcher` (`news-fetcher`) | bin | launchd-driven, twice-daily fetch + profile ranking (one-shot Sessions), writes SQLite and the notch widget's `news.json` (ADR-031). Posts nowhere. |
...
| **S5** | Preference learning loop — weekly job reads news upvote table, updates `news_preferences.md`. (Learner sunset by ADR-016. Superseded by ADR-031: the rubric is now a prose profile note the operator owns at `[news].profile_note`, revised in a monthly review conversation rather than regenerated by a job.) | ✅ shipped |
...
| **S10** | Gmail + Calendar via Claude.ai MCP, JARVIS persona at `messaging/gmail/`. Two halves: (a) reminders gain a `--channels calendar` route that spawns a JARVIS session and creates a Google Calendar event with `NUCLEUS_PERSONAL_EMAIL` as attendee, so any bot can land an invite on the user's main calendar; (b) daily `gmail-metabolism` launchd job sweeps unread mail on the configured trash account (`$NUCLEUS_GMAIL_ACCOUNT`), classifies into the locked `nucleus/*` taxonomy, TRASH-labels junk + kill-list senders, posts a one-line digest to Discord home. SQLite kill-list at `memory/gmail.db`. No raw Google API client — every call rides MCP. See ADR-007. | ✅ shipped |
