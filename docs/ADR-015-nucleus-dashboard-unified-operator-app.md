...
(`nucleus-dashboard`), served at a single origin (`web.example.com`).
...
Single origin (`web.example.com`), path-scoped:
...
1. **Phase 1 — Build everything (this work).** On feature branch
   ...
   - Add cloudflared route `web.example.com` → local port.
...
4. **Phase 4 — Tailscale gating ([[ADR-011]] proper).** Apply
   ...
   `web.example.com` (everything except `/news/api/*` and
