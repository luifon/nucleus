# ADR-009 — Canvas: agent-rendered interactive components in the dashboard chat
...
- **ADR-012 (proposed) — perimeter / private deployment.** The dashboard and chat surfaces must be moved behind Tailscale before canvas ships. News stays publicly accessible (read-only RSS-shape); dashboard and chat are operator-only. Canvas adds attack surface to a chat surface that is currently public; the perimeter move is therefore a hard precondition, not a parallel concern. ADR-012 is drafted at implementation kickoff, not now.
...
- Tailscale-only access (ADR-012 prerequisite) means the surface is not exposed to the public internet.

## Migration / rollout

1. **Prerequisite — ADR-012:** Tailscale-gate dashboard + chat. News stays public. Verify no leaks.
...
- **ADR-012 (perimeter)** — drafted at implementation kickoff. Tailscale ACLs, cloudflared adjustments, verifying that news stays public while dashboard + chat are private.
...
- ADR-012 (proposed) — perimeter / private deployment; hard prerequisite
